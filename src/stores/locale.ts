...
const LOCAL_STORAGE_SELECTED_LOCALE: string = 'selected-locale'

export const useLocaleStore = defineStore('locale', () => {
  const i18n = useI18n()
  const { availableLocales, locale } = i18n

  const selectedLocale = ref<string>(localStorage.getItem(LOCAL_STORAGE_SELECTED_LOCALE) ?? availableLocales[0] ?? '')
  ...
  const updateSelectedLocale = (newLocale: string): void => {
    ...
    localStorage.setItem(LOCAL_STORAGE_SELECTED_LOCALE, newLocale)
